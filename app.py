...
if opcion == "📅 Planificación Entregas":
    ...
    if not df.empty and "FECHA_ENTREGA_DT" in df.columns:
        ...
        # --- BOTONES DE ENTREGAS (CORREGIDO EL BOTÓN DEL MES) ---
        ...
        # AQUÍ ESTÁ EL CAMBIO: Quitamos el número confuso
        ...
        if st.session_state.modo_vista_agenda == 'entregados':
            ...
            if mapa_meses:
                ...
                if dia_filtro:
                    ...
                    # CAMBIO: Agregamos el contador aquí
...
# ==========================================
    ...
    if not df.empty and "FECHA_ARRIBO_DT" in df.columns:
        ...
        if not df_final.empty:
            st.subheader(titulo)
            st.info(mensaje)
            cols_base = ["VIN", "MARCA", "MODELO", "FECHA_ARRIBO_DT", "TAREA", "UB
