        ...
        if años:
            ...
            if not df_final.empty:
                ...
                if es_usados:
                    ...
                    # NUEVA CONFIGURACIÓN: Forzamos anchos estrictos en píxeles para compactar todo al 100%
                    config_columnas = {
                        "FECHA_ENTREGA_DT": st.column_config.DateColumn("Fecha", format="DD/MM/YYYY", width=90),
                        "HS DE ENTREGA AL CLIENTE": st.column_config.TextColumn("Hora", width=65),
                        "CLIENTE": st.column_config.TextColumn("Cliente", width=160),
                        "MARCA": st.column_config.TextColumn("Marca", width=80),
                        "MODELO": st.column_config.TextColumn("Modelo", width=160),
                        "CANAL DE VENTA": st.column_config.TextColumn("Canal de Venta", width=110),
                        "TELEFONO_CLEAN": st.column_config.TextColumn("Teléfono", width=100),
                        "VENDEDOR": st.column_config.TextColumn("Vendedor", width=120),
                    }
                    if col_admin: 
                        config_columnas[col_admin] = st.column_config.TextColumn("Estado Admin", width=180)
                    if col_vin_origen:
                        config_columnas[col_vin_origen] = st.column_config.TextColumn("Número de VIN / Chasis", width=145)
...
if opcion == "📅 Planificación Entregas 0KM":
    ...
    if not df_raw.empty:
        ...
        with tab_sub_tendencias:
            ...
            with col_g_btn1:
                if st.button("🚀 Vista Analítica: Con Fecha de Entrega", use_container_width=True, type=type_g1, key="btn_graf_con_fecha"):
                    st.session_state.filtro_grafico_segmento = '🚀 Vista: Con Fecha de Entrega'
            with col_g_btn2:
                if st.button("🚨 Vista Analítica: SIN Fecha de Entrega", use_container_width=True, type=type_g2, key="btn_graf_sin_fecha"):
            ...
            if st.session_state.filtro_grafico_segmento == '🚀 Vista: Con Fecha de Entrega':
                ...
                if not df_g_con.empty:
                    ...
                    with g_line1:
                        st.markdown("##### Promedio Días Pedido ➔ Entrega")
                    ...
                    with g_line2:
                        st.markdown("##### Promedio Días Papeles Disp. ➔ Entrega")
                ...
                if not df_g_pend.empty:
                    ...
                    with g_line1:
                        st.markdown("##### Promedio Días Pedido ➔ Hoy (Sin Entrega)")
                    ...
                    with g_line2:
                        st.markdown("##### Promedio Días Papeles Disp. ➔ Hoy (Sin Entrega)")
    ...
    if not df_0km.empty and "FECHA_ARRIBO_DT" in df_0km.columns:
        ...
        for index, row in df_mant.iterrows():
            ...
            for intervalo, columna in cols_control.items():
                ...
                if inicio_semana <= fecha_vencimiento <= fin_semana: motivos_semana.append(f"Control {intervalo} dias ({fecha_vencimiento.strftime('%d/%m')})")
                if hoy >= fecha_vencimiento: motivos_atrasados.append(f"Falta {intervalo} dias (Vencio: {fecha_vencimiento.strftime('%d/%m')})")
            if motivos_hoy: r = row.copy(); r["TAREA"] = ", ".join(motivos_hoy); lista_hoy.append(r)
            if motivos_semana: r = row.copy(); r["TAREA"] = ", ".join(motivos_semana); lista_semana.append(r)
            if motivos_atrasados: r = row.copy(); r["TAREA"] = motivos_atrasados[-1]; lista_atrasados.append(r)
